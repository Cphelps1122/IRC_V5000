...
def apply_theme():
    ...
    st.markdown(
        ...
        .delta-good {{ color: var(--accent-2); font-weight: 850; }}
        .delta-bad {{ color: var(--danger); font-weight: 850; }}
        .delta-watch {{ color: var(--warning); font-weight: 850; }}
        .delta-neutral {{ color: var(--text-main); font-weight: 850; }}
...
def _pick_delta_style(delta: float | None, direction: str = "lower_is_better", reference_delta: float | None = None) -> tuple[str, str]:
    """Return CSS class and arrow for a percent change.

    direction options:
    - lower_is_better: increases are bad, decreases are good.
    - higher_is_better: increases are good, decreases are bad.
    - treatment_adjusted: compares the metric movement to treatment movement.
      Cost/usage increases are green if they are justified by treatment growth,
      yellow if slightly above treatment growth, red if materially above.
    ...
    if direction == "treatment_adjusted":
        ...
        # A cost/usage increase that is within 5 points of treatment movement is acceptable.
        if gap <= 5:
            return ("delta-good" if d > 0 else "delta-good"), arrow
        if gap <= 10:
