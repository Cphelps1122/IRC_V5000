...
def normalize_data(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reporting month comes from the Month + Year columns only.
    service_month = _month_year_to_timestamp(
        out.get("month"),
        out.get("year"),
    )
    out["billing_month"] = service_month
...
def _month_year_to_timestamp(month_s: pd.Series, year_s: pd.Series) -> pd.Series:
    """Build the service/reporting month strictly from Month + Year."""
    index = month_s.index
    ...
    # Month can be numeric (6), abbreviated (Jun), full name (June), or a full date (6/1/2026).
    ...
    # If Month itself is a date, use the month/year from that date unless Year is supplied.
    date_like = month_text.str.contains(r"\d{1,4}[/-]\d{1,2}|\d{1,2}[/-]\d{1,4}", regex=True, na=False)
    full_month_dates = pd.Series(pd.NaT, index=index, dtype="datetime64[ns]")
    if date_like.any():
        full_month_dates.loc[date_like] = pd.to_datetime(month_text.loc[date_like], errors="coerce")
    month_num = month_num.fillna(full_month_dates.dt.month)

    year_num = pd.to_numeric(year_text.str.extract(r"(20\d{2}|19\d{2})", expand=False), errors="coerce")
    year_num = year_num.fillna(full_month_dates.dt.year)
