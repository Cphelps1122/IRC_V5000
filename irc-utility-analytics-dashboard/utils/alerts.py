...
def _latest_bill_text(combo_history: pd.DataFrame, current_month) -> str:
    ...
    if pd.notna(latest_date):
        return f"Latest bill on file: {pd.to_datetime(latest_date).strftime('%-m/%-d/%Y')}"
...
def build_alerts(df: pd.DataFrame, current_month, previous_month=None) -> pd.DataFrame:
    ...
    for _, r in merged.iterrows():
        ...
        rows.append({
            ...
            "Latest Bill": pd.to_datetime(r["billing_date"]).strftime("%-m/%-d/%Y") if pd.notna(r.get("billing_date")) else "—",
            "Explanation": explain_change(r["utility"], cost_chg, usage_chg, treatment_chg, cpt_chg),
        })

    # Missing bills for selected month only.
    expected = uagg[["state", "property", "utility"]].drop_duplicates()
    ...
    for _, combo in missing.iterrows():
        ...
        prev_rows = history[history["billing_month"] == previous_month] if previous_month is not None else history.iloc[0:0]
        prev_amount = prev_rows["amount"].sum() if not prev_rows.empty else np.nan
        reason = f"No bill found for selected month. {_latest_bill_text(history, current_month)}"
        rows.append({
            "Severity": "Critical",
            "State": combo["state"],
            ...
            "Reason": reason,
            "Current Cost": np.nan,
            "Previous Cost": prev_amount,
            "Cost Change %": np.nan,
            "Current Usage": np.nan,
            "Previous Usage": prev_rows["usage"].sum() if not prev_rows.empty else np.nan,
            "Usage Change %": np.nan,
            "Current Cost/Treatment": np.nan,
            "Previous Cost/Treatment": np.nan,
            "Cost/Treatment Change %": np.nan,
            "Current Cost/Usage": np.nan,
            "Previous Cost/Usage": np.nan,
            "Cost/Usage Change %": np.nan,
            "Treatments Change %": np.nan,
            "Estimated Monthly Impact": prev_amount if pd.notna(prev_amount) else 0,
            "Latest Bill": _latest_bill_text(history, current_month).replace("Latest bill on file: ", ""),
