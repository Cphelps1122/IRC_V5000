...
def build_alerts(df: pd.DataFrame, current_month, previous_month=None) -> pd.DataFrame:
    ...
    merged = cur.merge(prev[merge_cols + ["prev_amount", "prev_usage", "prev_treatments", "prev_cost_per_treatment", "prev_cost_per_usage"]], on=merge_cols, how="left")
    ...
    for _, combo in missing.iterrows():
        ...
        rows.append({
            ...
            "Missing Bill": True,
            "Explanation": "No selected-month bill was found for this property and utility. Confirm whether the bill is missing, delayed, or entered under a different reporting month.",
        })
