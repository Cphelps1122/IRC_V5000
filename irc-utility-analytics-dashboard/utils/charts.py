...
def geographic_figure(state_df: pd.DataFrame, property_df: pd.DataFrame):
    ...
    if state_df is not None and not state_df.empty:
        fig.add_trace(go.Choropleth(
            ...
            z=state_df["Properties"],
            locationmode="USA-states",
            colorscale="Blues",
            marker_line_color="rgba(255,255,255,.35)",
            colorbar_title="Properties",
            text=state_df.apply(lambda r: f"{r['State']}<br>Properties: {r['Properties']}<br>Alerts: {r['Alerts']}<br>Avg CPT: ${r['Cost/Treatment']:.2f}" if pd.notna(r.get('Cost/Treatment')) else f"{r['State']}<br>Properties: {r['Properties']}<br>Alerts: {r['Alerts']}", axis=1),
            hoverinfo="text",
            showscale=True,
        ))
