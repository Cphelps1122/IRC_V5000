...
def render_top_filters(df: pd.DataFrame, include_provider: bool = False, include_alert_level: bool = False) -> dict:
    ...
    months_desc = sorted([pd.to_datetime(m) for m in months], reverse=True)
    ...
    with st.container(key="sticky_filters"):
        ...
        with cols[0]:
            ...
            selected_label = st.selectbox("Current month", labels, index=default_idx, key="filter_current_month_v2")
