...
def generate_monthly_pdf(df, current_month, previous_month) -> bytes:
    ...
    story.append(Paragraph(f"{config.REPORT_TITLE}: {month_label}", title_style))
    story.append(Paragraph(f"{config.REPORT_SUBTITLE} | Previous month: {prev_label}", small))
    ...
    for takeaway in monthly_takeaways(df, current_month, previous_month, alerts)[:4]:
        story.append(Paragraph("- " + takeaway, small))
