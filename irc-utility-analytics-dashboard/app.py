...
with cols[2]:
    if len(selected_utilities) == 1:
        kpi_card("Cost / Usage", fmt_money(summary["current"]["cost_per_usage"], 4), fmt_money(summary["previous"]["cost_per_usage"], 4), summary["delta_cpu"], selected_utilities[0])
    else:
...
with right:
    ...
    if not breakdown.empty:
        ...
        view["% Change"] = view["% Change"].apply(lambda x: f"{x:+.1f}%" if pd.notna(x) else "—")
